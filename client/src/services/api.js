 ...
 * @param {string} token
 */
export const getChallenges = async (params = {}, token = null) => {
  const queryParams = new URLSearchParams();
  if (params.category && params.category !== 'All') queryParams.append('category', params.category);
  ...
  const queryStr = queryParams.toString();
  const endpoint = queryStr ? `/challenges?${queryStr}` : '/challenges';
 ...
 * @param {string} token
 */
export const getChallengeById = async (id, token = null) => {
  const headers = {};
  if (token && token !== 'null' && token !== 'undefined') {
    headers.Authorization = `Bearer ${token.trim()}`;
  }
...
/**
 * Select the best answer for a challenge (Admin only)
 * @param {string} id
 * @param {string} answerId
 * @param {string} token
 */
